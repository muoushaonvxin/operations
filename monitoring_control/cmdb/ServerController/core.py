	...
	def __create_cpu_component(self, ignore_errs=False):
		try:
			...
			if not len(self.response['error']) or ignore_errs == True:
				...
				obj.save()
				log_msg = "Asset[%s] ---> has added now [cpu] component with data [%s]" % (self.asset_obj, data_set)
	...
	def __create_disk_component(self):
		...
		if disk_info:
			for disk_item in disk_info:
				print(disk_item)
				try:
					# self.__verify_field(disk_item, 'slot', int)
		...
		for field in fields_from_db:
			...
			if val_from_data_source:
				...
				if val_from_db == val_from_data_source:
					...
					model_obj.save()
					log_msg = "Asset[%s] --> componet[%s] --> field[%s] has changed from [%s] to [%s]" % (self.asset_obj, model_obj, field, val_from_db, val_from_data_source)
